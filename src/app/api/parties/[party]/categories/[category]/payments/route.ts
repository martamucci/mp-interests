import { NextRequest, NextResponse } from 'next/server'
  ...
  function extractFromRawFields(
    ...
    for (const field of rawFields) {
      ...
      for (const searchName of fieldNames) {
        if (fieldName === searchName.toLowerCase() ||
  fieldName.includes(searchName.toLowerCase())) {
  ...
  export async function GET(_request: NextRequest, context: { params: { party:
    ...
    try {
      ...
      const enrichedPayments = (payments || []).map((payment) => {
        const typed = payment as {
          ...
          member:
            | { id: number; name_display: string; party_name: string;
  constituency: string | null }
            | Array<{ id: number; name_display: string; party_name: string;
  constituency: string | null }>
            | null
          interest:
            | { summary: string | null; raw_fields: unknown; registration_date:
  string | null }
            | Array<{ summary: string | null; raw_fields: unknown;
  registration_date: string | null }>
            | null
        }
        const member = Array.isArray(typed.member) ? typed.member[0] :
  typed.member
        const interest = Array.isArray(typed.interest) ? typed.interest[0] :
  typed.interest
        const rawFields = interest?.raw_fields as Array<{ name?: string; value?:
  string }> | null
        const date = typed.received_date || typed.start_date ||
  interest?.registration_date || null
        return {
          id: typed.id,
          amount: typed.amount,
  ...
  donation', 'description']),
      ...
      return NextResponse.json({ error: 'Failed to fetch payments' }, { status:
  500 })
    }
  }
